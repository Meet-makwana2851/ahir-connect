...
Deno.serve(async (req) => {
  try {
    ...
    if (userErr || !user) return new Response('Not authenticated', { status: 401, headers: corsHeaders });
    ...
    return new Response('Account deleted', { status: 200, headers: corsHeaders });
  } catch (err) {
